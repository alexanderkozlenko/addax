        ...
        if (typeReadingFieldsCount > 0)
        {
            ...
            for (var i = 0; i < fieldIndices.Length; i++)
            {
                ...
                if (fieldsToSkip > 0)
                {
                    ...
                    {
                        builder.AppendLine("            if (!reader.MoveNextField(cancellationToken))");
                        builder.AppendLine("            {");
                ...
                if (!recordSpec.IsStrict)
                {
                    builder.AppendLine("        if (!reader.ReadField(cancellationToken))");
                    builder.AppendLine("        {");
                    builder.AppendLine($"            return Tabular::TabularRecord<{recordSpec.TypeName}>.FromContent(new()");
                    builder.AppendLine("            {");
        ...
        if (typeWritingFieldsCount > 0)
        {
            ...
            for (var i = 0; i < fieldIndices.Length; i++)
            {
                ...
                if (i < recordSpec.FieldSpecs.Count - 1)
                {
                    ...
                    builder.AppendLine("        {");
                    builder.AppendLine("            writer.Flush(cancellationToken);");
                    builder.AppendLine("        }");
                    builder.AppendLine();
                }
            }
